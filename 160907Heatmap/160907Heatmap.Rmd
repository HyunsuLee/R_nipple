...
D33 <- read.csv("go_D33.csv", header = TRUE)
AD <- read.csv("go_AD.csv", header = TRUE)
RIB <- read.csv("go_RIB.csv", header = TRUE)
```
...
tiff(filename = "D33.tif", width = 5, height = 6.6, units = "in", res=300)
heatmap.2(D33_m, col=r, scale = "row", cexCol = 1, labRow = NA, key = TRUE, keysize = 1.5, trace = "none", density.info = "none", dendrogram = "none", Rowv=cluCOM_dend_D33, Colv = FALSE)
dev.off()
```

   AD signature
```{r, echo=FALSE}
tiff(filename = "AD.tif", width = 5, height = 8.2, units = "in", res=300)
...
```
   ...
   RIB signature
```{r, echo=FALSE}
tiff(filename = "RIB.tif", width = 5, height = 8.5, units = "in", res=300)
