...
colnames(human_sub) <- c("id","gene.sym", "gene.title", "CS1", "CS2", "CS3","CS4","Skin_Bd","Skin_Sg", "Skin4","Skin1", "Skin5_1","Skin5_2","Skin10", "MamGld10","MamGld9")
...
colnames(human_raw_sub) <- c("id","gene.sym", "gene.title", "CS1", "CS2", "CS3","CS4","Skin_Bd","Skin_Sg","Skin1.Ds", "Skin4.Nr","Skin1.Nr", "Skin2.Ds", "Skin3.Ds", "Skin5_1","Skin5_2","MamGld10","MamGld9", "Skin10.Nr")
...
colnames(composite) <- c("gene.sym", "gene.desc", "Str.vehicle.rep1", "Str.vehicle.rep2", "Str.vehicle.rep3","Str.estrogen.rep1","Str.estrogen.rep2","Str.estrogen.rep3", "krp1", "krp2", "krp3", "wt1", "wt2","wt3")
...
ggplot(human.pca.scores, aes(PC1, PC2)) + 
  ...
  geom_point(color="#FFFFFF", size = 1.5, show.legend = TRUE) +
  # Point 오른쪽에 text 추가
  geom_text_repel(aes(label=rownames(human.pca.scores)), size =5, nudge_x= 25, show.legend = FALSE) +
  # 배경을 하얀색으로, axis label size과 title size 조절
  theme_bw() + theme(axis.text =element_text(size = rel(1.2))) +
  theme(axis.title=element_text(size=rel(1.5))) +
  # legend tilte없애고, box를 하얀색으로 안보이게, legend size조절.
...
ggplot(human.raw.pca.scores, aes(PC1, PC2)) + 
  ...
  geom_point(color="#FFFFFF", size = 1.5, show.legend = TRUE) +
  # Point 오른쪽에 text 추가
  geom_text_repel(aes(label=rownames(human.raw.pca.scores)), size =5, nudge_x= 25, show.legend = FALSE) +
  # 배경을 하얀색으로, axis label size과 title size 조절
  theme_bw() + theme(axis.text =element_text(size = rel(1.2))) +
  theme(axis.title=element_text(size=rel(1.5))) +
  # legend tilte없애고, box를 하얀색으로 안보이게, legend size조절.
...
ggplot(human.raw.log.pca.scores, aes(PC1, PC2)) + 
  ...
  geom_point(color="#FFFFFF", size = 1.5, show.legend = TRUE) +
  # Point 오른쪽에 text 추가
  geom_text_repel(aes(label=rownames(human.raw.log.pca.scores)), size =5, nudge_x= 25, show.legend = FALSE) +
  # 배경을 하얀색으로, axis label size과 title size 조절
  theme_bw() + theme(axis.text =element_text(size = rel(1.2))) +
  theme(axis.title=element_text(size=rel(1.5))) +
  # legend tilte없애고, box를 하얀색으로 안보이게, legend size조절.
...
## Composite(KrP, mouse estrogen) PCA and plotting.
```{r, echo=FALSE}
composite.pca <- prcomp(composite.t, scale. =TRUE) # PCA 시행.
summary(composite.pca)
# screeplot(nipple.pca) PC들의 분산들에 대해 plot. 이것 보단 variance propotion이 나은듯.
composite.pca.var = composite.pca$sdev ^ 2 # 주성분들의 분산 계산
composite.pca.pve = composite.pca.var / sum(composite.pca.var) #주성분 분산의 propotion 계산
plot (composite.pca.pve, xlab = "Principal Component", ylab = "Propotion of Variance Explained", ylim = c(0,1), type = "b")
composite.pca.scores = data.frame(composite.pca$x) #PC들만 따로 뽑기.
...
composite.pca.scores <- cbind(composite.pca.scores, group_com, rep_com) # PC matrix에 data labeling


ggplot(composite.pca.scores, aes(PC1, PC2)) + 
  # point layer 추가, 이중 point 로 그림 그리기
  geom_point(aes(color=group_com), size = 4) +
  geom_point(color="#FFFFFF", size = 1.5, show.legend = TRUE) +
  # Point 오른쪽에 text 추가
  geom_text_repel(aes(label=rep_com, color= group_com), size =5, nudge_x= 25, show.legend = FALSE) +
  # 배경을 하얀색으로, axis label size과 title size 조절
  theme_bw() + theme(axis.text =element_text(size = rel(1.2))) +
  theme(axis.title=element_text(size=rel(1.5))) +
  # legend tilte없애고, box를 하얀색으로 안보이게, legend size조절.
